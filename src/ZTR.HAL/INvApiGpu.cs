...
public interface INvApiGpu
{
    ...
    /// <summary>
    /// Gets the number of physical NVIDIA GPUs detected.
    /// </summary>
    int GpuCount { get; }
    ...
    /// <summary>
    /// Gets the list of supported power mode names.
    /// </summary>
    /// <param name="index">The GPU index.</param>
    /// <returns>A read-only list of power mode names.</returns>
    IReadOnlyList<string> GetSupportedPowerModes(int index);

    /// <summary>
    /// Sets the maximum GPU clock speed.
    /// </summary>
    /// <param name="index">The GPU index.</param>
    /// <param name="clock">The maximum clock speed in MHz.</param>
    /// <returns>True if the operation succeeded; otherwise false.</returns>
    bool SetMaxGpuClock(int index, int clock);
